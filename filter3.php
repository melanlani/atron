<?php
  ...
  if($_POST['request'] && $_POST['req']){
    $request = $_POST['request'];
    $up = $_POST['req'];
    $conn = mysqli_connect('localhost','root','','atron');
    $query="select regional, city, site_id, site_name, bw, status, current, aging, today_highest, weekly_highest, monthly_highest, yearly_highest from current_occupancy where status='$up' and regional='$request' group by current_id";
    $result=mysqli_query($conn,$query);
    echo '<div class="row">';
            echo '<div class="col-md-12">';
                echo '<div class="main-card mb-3 card">';
                    echo '<div class="card-header">NODE B OVERVIEW > NODE B STATUS > UP</div>';
